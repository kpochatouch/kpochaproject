  ...
  useEffect(() => {
    const normalized = (initialMessages || []).map((m) => {
      ...
      if (!status) {
        if (m.isMe) {
          const seen =
            Array.isArray(m.seenBy) && peerUid && m.seenBy.includes(peerUid);
      ...
      <div className="flex-1 overflow-y-auto overflow-x-hidden space-y-2 p-3 border border-zinc-800 rounded-xl no-scrollbar">
        {msgs.map((m, i) => {
          ...
          if (isMe) {
            ...
            displayVerified = !!m.sender.verified;
          } else if (
            peerProfile &&
            ...
            displayVerified = !!peerProfile.verified;
          } else {
            displayName = "Unknown";
            displayVerified = false;
          }
            ...
            <div key={m.id || i}>
              ...
              <div className={`flex ${isMe ? "justify-end" : "justify-start"}`}>
                ...
                >
                  ...
                  {reaction && <div className="mt-1 text-xs">{reaction}</div>}
