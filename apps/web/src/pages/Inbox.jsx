...
export default function Inbox() {
  ...
  const myUid =
    currentUser?.uid || currentUser?.ownerUid || currentUser?._id || null;
  ...
  useEffect(() => {
    ...
    async function hydrateMissingProfiles() {
      ...
      const missing = threads.filter(
        (t) =>
          t.peerUid && (!t.displayName || t.displayName === "Unknown user"),
      );
      ...
      for (const uid of uniqueUids) {
        try {
          ...
          console.warn(
            "[Inbox] hydrate profile failed for",
            uid,
            e?.message || e,
          );
        }
  ...
  async function openThread(t) {
    ...
    try {
      ...
      console.warn(
        "[Inbox] markThreadRead/markRoomRead failed:",
        e?.message || e,
      );
    }
  }
